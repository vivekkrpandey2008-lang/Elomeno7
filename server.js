...
const ADMIN_PASSCODE = String(process.env.ADMIN_PASSCODE || '').trim().replace(/^['"]|['"]$/g, '') || 'Elomeno7@1234';
const ADMIN_FALLBACK_PASSCODE = 'Elomeno7@1234';
...
// A student's own result history (used by the "My Progress" tab).
// No passcode needed — matches the existing login model (name + roll only).
app.get('/api/results/:roll', async (req, res) => {
  try {
    ...
    `, [req.params.roll]);
    res.json(result.rows);
  } catch (e) { console.error(e); res.status(500).json({ error: 'Could not load your results.' }); }
});
