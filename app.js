...
const state = {
  ...
  adminSubject:null, adminQuestions:[], results:[], search:''
};
...
function render(){
  stopTimer();
  const views = {landing:landing,signup:authSignup,login:authLogin,dashboard:dashboard,test:test,result:result,adminGate:adminGate,admin:admin};
  app.innerHTML = views[state.screen]();
  bind();
}
function logo(dark=true){ return `<div class="logo"><div class="logo-mark">E7</div><span>Elomeno<b>7</b> Study</span></div>`; }

function landing(){
 return `<div class="hero">
   <div class="topbar">${logo()}<button class="btn btn-outline" style="color:#fff;border-color:#ffffff2b" id="adminBtn">Admin</button></div>
   <div class="hero-main container">
     <div>
       <span class="eyebrow">✦ Smart Study • Practice • Improve</span>
       <h1>Prepare better.<br><span>Test smarter.</span></h1>
       <p class="hero-copy">Elomeno7 Study is a clean, distraction-free test portal where students can practice different subjects, track their scores and build confidence one test at a time.</p>
       <div class="hero-actions"><button class="btn btn-primary" id="signupBtn">Get started →</button><button class="btn btn-outline" style="color:#fff;border-color:#ffffff35" id="loginBtn">I already have an account</button></div>
       <div class="hero-stats"><div class="hero-stat"><b>${state.subjects.length || '—'}</b><span>Subjects</span></div><div class="hero-stat"><b>MCQ</b><span>Practice format</span></div><div class="hero-stat"><b>24/7</b><span>Access</span></div></div>
     </div>
...
function authShell(title, sub, fields, button, footer){
 return `<div class="auth-page"><div class="auth-card"><button class="back" id="backHome">← Back to home</button><div class="auth-logo">${logo(false)}</div><h1>${title}</h1><p class="sub">${sub}</p>${state.error?`<div class="msg error">${esc(state.error)}</div>`:''}${state.info?`<div class="msg success">${esc(state.info)}</div>`:''}${fields}<button class="btn btn-primary" style="width:100%;margin-top:8px" id="submitAuth">${button}</button><div style="text-align:center;margin-top:17px;font-size:13px;color:#667085">${footer}</div></div></div>`;
}
function authSignup(){ return authShell('Create your account','One-time registration. Use your name and roll number for future logins.',
 ...
 'Create account','Already registered? <button class="back" style="display:inline;color:#5b4bd8;font-weight:700" id="switchLogin">Login</button>');}
function authLogin(){ return authShell('Welcome back','Enter your registered details to continue.',
 `<div class="field"><label>FULL NAME</label><input id="name" placeholder="e.g. Vivek Kumar"></div><div class="field"><label>ROLL NUMBER</label><input id="roll" placeholder="e.g. 01"></div>`,
 'Login →','New here? <button class="back" style="display:inline;color:#5b4bd8;font-weight:700" id="switchSignup">Create account</button>');}
...
function dashboard(){
 ...
 return `<div class="page"><div class="nav-dark"><div class="nav-inner">${logo()}<div class="nav-actions"><span class="user-pill">${esc(state.user.name)} · ${esc(state.user.roll)}</span><button class="btn btn-outline" style="color:#fff;border-color:#ffffff20" id="logout">Logout</button></div></div></div>
 ...
 <main class="container dashboard-body"><div class="toolbar">⌕ <input class="search" id="search" value="${esc(state.search)}" placeholder="Search subjects..."><span style="font-size:12px;color:#8a93a3">${filtered.length} available</span></div>
 <h2 class="section-title">All subjects</h2>${filtered.length?`<div class="subject-grid">${filtered.map(subjectCard).join('')}</div>`:`<div class="empty">No subject matches your search.</div>`}</main></div>`;
}
function subjectCard(s){ return `<article class="subject"><div class="subject-icon">${esc(s.icon)}</div><h3>${esc(s.name)}</h3><p>${esc(s.description)}</p><div class="subject-meta"><span>${s.questionCount} questions</span><span>⏱ ${s.durationSeconds}s / Q</span></div><button class="btn btn-primary" style="width:100%" data-start="${esc(s.slug)}" ${!s.questionCount?'disabled':''}>${s.questionCount?'Start test →':'Coming soon'}</button></article>`; }
...
function result(){const total=state.questions.length,pct=Math.round(state.score/total*100);return `<div class="result-page"><div class="result-card"><div style="color:#667085;font-size:12px;font-weight:700">TEST COMPLETE</div><div class="score"><b>${pct}%</b><span>YOUR SCORE</span></div><h1 style="font-size:23px">Nice work, ${esc(state.user.name)}!</h1><p class="sub" style="margin-bottom:0">${esc(state.subject.name)} practice test completed.</p><div class="result-stats"><div class="stat"><b style="color:#16a34a">${state.score}</b><span>Correct</span></div><div class="stat"><b style="color:#dc2626">${total-state.score}</b><span>Wrong</span></div><div class="stat"><b>${total}</b><span>Total</span></div></div><button class="btn btn-primary" style="width:100%" id="backDash">Back to dashboard</button></div></div>`}

function adminGate(){return `<div class="auth-page"><div class="auth-card"><button class="back" id="backHome">← Back to home</button><div class="auth-logo">${logo(false)}</div><h1>Admin access</h1><p class="sub">Manage subjects, upload questions and review student results.</p>${state.error?`<div class="msg error">${esc(state.error)}</div>`:''}<div class="field"><label>ADMIN PASSCODE</label><input id="pass" type="password" placeholder="Enter passcode"></div><button class="btn btn-primary" style="width:100%" id="adminLogin">Open admin panel</button></div></div>`}

function admin(){
 const tabs=['subjects','questions','results'];let body='';
 ...
 return `<div class="admin"><header class="admin-head"><div class="nav-inner">${logo()}<button class="btn btn-outline" style="color:#fff;border-color:#ffffff20" id="adminExit">Exit</button></div></header><main class="admin-main"><div class="admin-tabs">${tabs.map(t=>`<button class="tab ${state.adminTab===t?'active':''}" data-tab="${t}">${t[0].toUpperCase()+t.slice(1)}</button>`).join('')}</div>${body}</main></div>`;
}
function adminSubjects(){return `<div class="admin-grid"><section class="panel"><h2>Add a new subject</h2><div class="admin-form"><div class="field"><label>SUBJECT NAME</label><input id="sName" placeholder="e.g. Economics"></div><div class="field"><label>ICON</label><input id="sIcon" placeholder="📊" maxlength="4"></div><div class="field full"><label>DESCRIPTION</label><input id="sDesc" placeholder="Short description"></div><div class="field"><label>TIME / QUESTION (SECONDS)</label><input id="sTime" type="number" value="30" min="10"></div></div><button class="btn btn-primary" style="width:100%;margin-top:8px" id="addSubject">+ Add subject</button><div class="hint">A slug is generated automatically. After adding a subject, open Questions to upload its MCQs.</div></section><section class="panel"><h2>Subjects</h2><div class="list">${state.subjects.map(s=>`<div class="list-item"><div><b>${esc(s.icon)} ${esc(s.name)}</b><div class="small">${s.questionCount} questions · ${s.durationSeconds}s/Q</div></div><button class="btn btn-danger" data-delete-subject="${esc(s.slug)}">Delete</button></div>`).join('')}</div></section></div>`}
function adminQuestions(){const s=state.subjects.find(x=>x.slug===state.adminSubject)||state.subjects[0];if(!s)return '<div class="empty">Add a subject first.</div>';state.adminSubject=s.slug;return `<div class="panel"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap"><h2>Question manager</h2><select class="select" style="width:auto" id="subjectSelect">${state.subjects.map(x=>`<option value="${esc(x.slug)}" ${x.slug===s.slug?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div><textarea id="bulk" rows="11" placeholder="Question text
Option 1
Option 2
Option 3
Option 4
...
Leave one blank line between questions."></textarea><div class="hint"><b>Format:</b> 6 lines per question — question, 4 options, correct number. Separate questions with a blank line.</div><button class="btn btn-primary" style="margin-top:12px" id="addQuestions">+ Upload questions</button></div><div class="panel"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h2>${state.adminQuestions.length} saved questions</h2>${state.adminQuestions.length?'<button class="btn btn-danger" id="clearQuestions">Clear all</button>':''}</div><div class="list">${state.adminQuestions.map((q,i)=>`<div class="list-item"><div><b>${i+1}. ${esc(q.text)}</b><div class="small">Correct: ${esc(q.options[q.correctIndex])}</div></div><button class="btn btn-danger" data-delete-q="${q.id}">Delete</button></div>`).join('') || '<div class="empty">No questions yet.</div>'}</div></div>`}
function adminResults(){const names=Object.fromEntries(state.subjects.map(s=>[s.slug,s.name]));return `<div class="panel"><div style="display:flex;justify-content:space-between;align-items:center;gap:10px"><h2>Student results</h2><select class="select" style="width:auto" id="resultFilter"><option value="">All subjects</option>${state.subjects.map(s=>`<option value="${esc(s.slug)}">${esc(s.name)}</option>`).join('')}</select></div><div style="overflow:auto"><table class="result-table"><thead><tr><th>Student</th><th>Subject</th><th>Score</th><th>Date</th></tr></thead><tbody>${state.results.filter(r=>!$('#resultFilter')?.value||r.subject===$('#resultFilter').value).map(r=>`<tr><td><b>${esc(r.name)}</b><br><span class="small">Roll ${esc(r.roll)}</span></td><td>${esc(r.subjectName||names[r.subject]||r.subject)}</td><td><b>${r.score}/${r.total}</b> (${Math.round(r.score/r.total*100)}%)</td><td>${new Date(r.timestamp).toLocaleString()}</td></tr>`).join('')||'<tr><td colspan="4">No results yet.</td></tr>'}</tbody></table></div></div>`}
...
async function loadAdmin(){await loadSubjects();state.results=await api('/api/results');if(state.adminSubject)await loadAdminQuestions();render()}
...
function bind(){
 ...
 $('#adminLogin')?.addEventListener('click',async()=>{try{const pass=$('#pass').value;await api('/api/admin/verify',{method:'POST',body:JSON.stringify({passcode:pass})});state.adminPass=pass;state.adminTab='subjects';state.adminSubject=null;await loadAdmin()}catch(e){state.error=e.message;render()}});
 $('#adminExit')?.addEventListener('click',()=>{state.adminPass=null;state.screen='landing';render()});
 document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',async()=>{state.adminTab=b.dataset.tab;if(state.adminTab==='results')state.results=await api('/api/results');if(state.adminTab==='questions'){if(!state.adminSubject)state.adminSubject=state.subjects[0]?.slug;await loadAdminQuestions()}render()}));
