...
function adminResults(){const names=Object.fromEntries(state.subjects.map(s=>[s.slug,s.name]));const filtered=state.results.filter(r=>!state.resultFilter||r.subject===state.resultFilter);return `<div class="panel"><div style="display:flex;justify-content:space-between;align-items:center;gap:10px"><h2>Student results</h2><select class="select" style="width:auto" id="resultFilter"><option value="">All subjects</option>${state.subjects.map(s=>`<option value="${esc(s.slug)}" ${s.slug===state.resultFilter?'selected':''}>${esc(s.name)}</option>`).join('')}</select></div><div style="overflow:auto"><table class="result-table"><thead><tr><th>Student</th><th>Subject</th><th>Score</th><th>Date</th></tr></thead><tbody>${filtered.map(r=>`<tr><td><b>${esc(r.name)}</b><br><span class="small">Roll ${esc(r.roll)}</span></td><td>${esc(r.subjectName||names[r.subject]||r.subject)}</td><td><b>${r.score}/${r.total}</b> (${Math.round(r.score/r.total*100)}%)</td><td>${new Date(r.timestamp).toLocaleString()}</td></tr>`).join('')||'<tr><td colspan="4">No results yet.</td></tr>'}</tbody></table></div></div>`}
...
function bind(){
 ...
 document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',async()=>{state.adminTab=b.dataset.tab;if(state.adminTab==='results'){try{state.results=await api('/api/results');state.resultsError=''}catch(e){state.results=[];state.resultsError=e.message||'Results could not be loaded.'}}if(state.adminTab==='questions'){if(!state.adminSubject)state.adminSubject=state.subjects[0]?.slug;try{await loadAdminQuestions()}catch(e){state.adminQuestions=[];toast(e.message)}}render()}));
